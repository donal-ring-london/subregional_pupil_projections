...
ets_projections <- mapply( # adding input range years as a variable
  FUN = function(input_data, input_year_range) {
    return(input_data[, input_year_range := input_year_range])
  },
...
calculate_mape(
  real_series = ets_projections[, headcount],
  projected_series = ets_projections[, mean_projection]
)

calculate_mape(
  real_series = pa_projections[, headcount],
  projected_series = pa_projections[, mean_projection]
)


calculate_percentage_within_intervals(
  real_series = ets_projections[, headcount],
...
png(
  file = "plots/adhoc_plots/mape.png",
  height = 8, width = 12, units = "in", res = 600
)

ggplot() +
  geom_bar(
    data = both_mape_itl, stat = "identity", position = position_dodge(width = 0.7),
...
png(
  file = "plots/adhoc_plots/perc_within.png",
  height = 8, width = 12, units = "in", res = 600
)

ggplot() +
  geom_bar(
    data = both_within_itl, stat = "identity", position = position_dodge(width = 0.7),
...
png(
  file = "plots/adhoc_plots/mape_by_year.png",
  height = 8, width = 12, units = "in", res = 600
)

ggplot() +
  geom_bar(
    data = both_mape_byyear, stat = "identity", position = position_dodge(width = 0.7),
...
png(
  file = "plots/adhoc_plots/perc_within_by_year.png",
  height = 8, width = 12, units = "in", res = 600
)

ggplot() +
  geom_bar(
    data = both_within_byyear, stat = "identity", position = position_dodge(width = 0.7),
