...
for (nc_year_select in nc_year_vec) {
  ...
  for (geog in unique_geogs) {
    ...
    lines(
      x = 2011:(max_year - 1), y = projections_nc[itl22cd == geog & year %in% 2011:(max_year - 1), mean_projection],
      col = "blue", lwd = 3
    )

    polygon(
      x = c(max_year:max_output_year, max_output_year:max_year), y = c(
        projections_nc[itl22cd == geog & year %in% max_year:max_output_year, upper_pi],
    ...
    lines(
      x = max_year:max_output_year, y = projections_nc[itl22cd == geog & year %in% max_year:max_output_year, mean_projection],
      col = "red", lwd = 0.5
    )
  ...
  legend("topright",
    legend = c("Headcount", "Projected headcount"),
