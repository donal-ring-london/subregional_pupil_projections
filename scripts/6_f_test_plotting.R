...
for(i in 1:length(all_geogs)){
  ...
  png(file = paste0("plots/", max_year, "_", max_year + 9 ,"/reception_ratio_diag/", geog_name, ".png"),
     height = 7, width = 12, units = "in", res = 600)

  ylim_min <- min(input_bootstrapped[[geog]], input_real[itl_cd == geog, ratio], 
                 output_bootstrapped[[geog]], output_central[[geog]])

  ylim_max <- max(input_bootstrapped[[geog]], input_real[itl_cd == geog, ratio], 
                  output_bootstrapped[[geog]], output_central[[geog]])

   ### setting the plot frame
  plot(x = min_input_year:max_output_year, y = rep(1, length(min_input_year:max_output_year)),
  ...
  for(j in 1:nrow(input_bootstrapped[[geog]])){
    ...
    lines(x = min_input_year:max_input_year, y = input_bootstrapped[[geog]][j, ],
         col = input_boot_col, lwd = 0.1)
  
  }

   ### plotting the input series
  lines(x = min_input_year:max_input_year, y = input_real[itl_cd == geog, ratio],
       col = "red", lwd = 2)

   ### plotting the output bootstraps
  for(j in 1:nrow(output_bootstrapped[[geog]])){
   
   lines(x = min_output_year:max_output_year, y = output_bootstrapped[[geog]][j, ],
         col = output_boot_col, lwd = 0.1)
  
  }

    ### plotting the output series
  lines(x = min_output_year:max_output_year, y = output_central[[geog]],
       col = "blue", lwd = 2)
