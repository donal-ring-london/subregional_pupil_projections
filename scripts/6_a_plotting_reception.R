...
projections_to_read <- paste0("output_projections/initial_tenyear/reception_projections_", max_year, "_", max_year + 9, "_ratio_ets.csv")
...
for(geog in unique_geogs){
  ...
  projection_line_col <- rgb(255, 0, 0, alpha = 255, maxColorValue = 255)
    ...
    #### making the plot
  png(file = paste0("plots/", max_year, "_", max_year + 9 ,"/reception_tenyear/", geog_name, ".png"),
  ...
  legend("topright", legend = c("Reception headcount", "Projected reception headcount", "95% prediction intervals"),
        ...
        lwd = c(3, 0.5, NA), fill = c(NA, NA, polygon_colour), border = NA)
...
png(file = paste0("plots/", max_year, "_", max_year + 9, "/all_years_tenyear_forecast/reception_tenyear.png"),
